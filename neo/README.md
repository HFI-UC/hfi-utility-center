...
页面采用纯白背景、扁平排版和 Astryx Design System。姓名、学号和邮箱前缀直接
在句子中填写；用户点击其他下划线内容后，对应选择区会纵向展开并将后续文字
推开，选择完成后仍可再次点击修改。

## 交互流程

1. 页面先显示 3 秒 `HFI Utility Center` 开屏标题。
2. 直接在句子中填写姓名、学号和 `@gdhfi.com` 邮箱前缀，无需展开输入区。
...
- **展示层**：`App.tsx` 使用一个连续句子承载预约信息。身份信息使用原生内联输入，
  需要选择的词语对应 `activePanel`，展开区通过 CSS Grid 与 Flex 布局把后续文字
  向下推开。
