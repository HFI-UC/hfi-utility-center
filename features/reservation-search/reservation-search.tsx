...
const statusClass: Record<ReservationStatus, string> = {
  pending: "border-amber-500 bg-amber-50 text-amber-950 dark:bg-amber-950 dark:text-amber-100",
  approved: "border-emerald-700 bg-emerald-700 text-white",
...
export function ReservationSearch() {
  ...
  return (
    <main className="mx-auto max-w-[96rem] px-4 py-10 sm:px-8">
      <div className="flex flex-col justify-between gap-6 border-t border-foreground/20 py-5 sm:flex-row sm:items-end">
        <div><p className="swiss-label">{t("eyebrow")}</p><h1 className="mt-3 text-4xl font-semibold leading-none sm:text-5xl">{t("title")}</h1></div>
        <p className="border-l-2 border-brand pl-4 text-xl font-semibold sm:text-2xl">{t("total", { count: result.total })}</p>
      </div>

      <div className="grid gap-3 border-y border-foreground/15 py-5 md:grid-cols-6">
        ...
        <select className="h-11 rounded-md border border-foreground/25 bg-background px-3 text-sm focus-visible:border-brand focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-brand" value={roomId || ""} onChange={(event) => updateQuery({ room: event.target.value || undefined })}><option value="">{t("allRooms")}</option>{catalog?.rooms.map((room) => <option key={room.id} value={room.id}>{room.name}</option>)}</select>
        <select className="h-11 rounded-md border border-foreground/25 bg-background px-3 text-sm focus-visible:border-brand focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-brand" value={status ?? ""} onChange={(event) => updateQuery({ status: event.target.value || undefined })}><option value="">{t("allStatuses")}</option><option value="pending">{statusT("pending")}</option><option value="approved">{statusT("approved")}</option><option value="rejected">{statusT("rejected")}</option></select>
