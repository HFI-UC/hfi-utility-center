...
function Bars({ values, labels }: { values: number[]; labels: string[] }) {
  ...
  return <div className="space-y-2">{values.map((value, index) => <div key={`${labels[index]}-${index}`} className="grid grid-cols-[3rem_1fr_3rem] items-center gap-3 text-xs"><span className="text-muted-foreground">{labels[index]}</span><div className="h-2 rounded-full bg-muted"><div className="h-full rounded-full bg-brand" style={{ width: `${Math.max(value ? 4 : 0, value / max * 100)}%` }} /></div><span className="text-right font-mono">{value}</span></div>)}</div>
}
...
export function AnalyticsAdmin() {
  ...
  return <main><AdminPageHeader eyebrow="ANALYTICS" title="数据分析" description="查看今日运营概览、近 30 天趋势和上一完整周的房间使用情况。" actions={<Button variant="outline" onClick={() => void load()}><RefreshCw />刷新</Button>} />{error ? <p className="mt-5 border-y py-3 text-sm text-destructive">{error}</p> : null}
