...
export function ReservationsAdmin() {
  ...
  return (
    <main>
      ...
      {error ? <p className="border-y py-3 text-sm text-red-600">{error}</p> : null}
      ...
      <div className="divide-y border-t">
        {filtered.map((item) => (
          <article key={item.id} className="grid gap-4 py-5 xl:grid-cols-[8rem_1.2fr_1fr_1.2fr_auto] xl:items-start">
            ...
            {rejectingId === item.id ? <div className="grid gap-3 border-l-2 border-red-600 pl-4 xl:col-span-5"><Textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="填写拒绝原因，该内容会通知预约人" /><div className="flex gap-2"><Button variant="destructive" disabled={workingId === item.id} onClick={() => void decide(item.id, false)}>确认拒绝</Button><Button variant="outline" onClick={() => { setRejectingId(undefined); setReason("") }}>取消</Button></div></div> : null}
          </article>
        ))}
      </div>
    </main>
