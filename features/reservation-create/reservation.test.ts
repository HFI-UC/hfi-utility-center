...
describe("预约分步规则", () => {
  it("接受姓名和普通邮箱并拒绝错误学号", () => {
    ...
    expect(reservationSchema.safeParse({ ...valid, studentName: "张三", email: "student@gdhfi.com" }).success).toBe(true)
    expect(reservationSchema.safeParse({ ...valid, studentName: "Andy Chen", email: "andy@example.com" }).success).toBe(true)
...
describe("旧版 API 兼容", () => {
  it("使用旧预约和房间政策生成 15 分钟空闲时段", () => {
    ...
    const reservations = [{ id: 3, studentName: "Andy", email: "student.andy2024@gdhfi.com", startTime: "2026-07-23T09:00:00", endTime: "2026-07-23T10:00:00", reason: "Meeting", status: "approved" }] satisfies Reservation[]
