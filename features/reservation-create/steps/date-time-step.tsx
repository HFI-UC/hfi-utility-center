...
import type { AvailabilityData, AvailabilitySlot } from "@/lib/api/types"
...
export function DateTimeStep() {
  ...
  async function load() {
    ...
    try { setAvailability(special === "auditorium" ? auditoriumAvailability(date) : await getAvailability(room, date)) }
  ...
  useEffect(() => {
    ...
    Promise.resolve().then(() => {
      ...
      return special === "auditorium" ? auditoriumAvailability(date) : getAvailability(room, date)
  ...
  return <StepLayout eyebrow="03 / 05" title={`${t("dateTitle")} · ${t("timeTitle")}`} description={t("timeDescription")} error={error ?? formState.errors.date?.message ?? formState.errors.startTime?.message ?? formState.errors.endTime?.message}>
    ...
    {date ? <><div className="mb-5 mt-8 flex items-center justify-between border-b pb-3 text-xs text-muted-foreground"><div className="flex flex-wrap gap-4"><span>● {t("available")}</span><span className="text-amber-600">● {t("policy")}</span><span>● {t("occupied")}</span></div><Button type="button" variant="ghost" size="icon-sm" onClick={() => void load()} title={t("refresh")}><RefreshCw className={cn(loading && "animate-spin")} /></Button></div>
      {loading ? <p className="py-10 text-sm text-muted-foreground">{t("checking")}</p> : null}
      {!loading && availability ? <div className="grid grid-cols-4 gap-2 sm:grid-cols-6 lg:grid-cols-8">{availability.slots.map((slot) => <button type="button" key={slot.startTime} disabled={slot.status !== "available"} aria-pressed={slot.startTime === startTime} onClick={() => { setValue("startTime", slot.startTime, { shouldValidate: true }); setValue("endTime", 0) }} className={cn("h-11 border text-sm focus-visible:ring-2 focus-visible:ring-red-600 disabled:bg-muted disabled:text-muted-foreground", slot.status === "available" && "border-emerald-600 text-emerald-700", slot.status === "policy" && "border-amber-300 bg-amber-50 text-amber-700", slot.startTime === startTime && "bg-foreground text-background")}>{timeLabel(slot.startTime)}</button>)}</div> : null}
