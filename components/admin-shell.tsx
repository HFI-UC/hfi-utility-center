    ...
    <div className="mx-auto grid max-w-[96rem] gap-0 px-4 py-8 sm:px-8 lg:grid-cols-[15rem_1fr]">
      <aside className="border-b pb-5 lg:min-h-[calc(100svh-8rem)] lg:border-r lg:border-b-0 lg:pr-6">
        ...
        <nav className="grid grid-cols-2 gap-1 sm:grid-cols-4 lg:grid-cols-1">
          {items.map((item, index) => <Link key={item.href} href={item.href} className={cn("flex h-11 items-center gap-3 rounded-md px-3 text-sm font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground", pathname === item.href && "bg-accent text-foreground")}><span className="font-mono text-[0.625rem] text-muted-foreground">0{index + 1}</span><item.icon className="size-4" />{item.label}</Link>)}
        </nav>
