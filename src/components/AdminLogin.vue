...
onMounted(async () => {
    if (props.requireLogin) {
        ...
        if (!response.success) {
            ...
            setTimeout(() => window.location.href = `/admin/login?redirect=${encodeURIComponent(window.location.href)}`, 2500);
        }
    }
    else {
        const response = await getCheckLogin();
        if (response.success) {
            ...
            setTimeout(() => window.location.href = props.redirect || "/admin/dashboard", 2500);
        }
    }
})
</script>
