...
import { getAdvertisementPrice, postEditAdvertisement, getAdvertisement } from "@/api";
...
const adId = computed(() => parseInt(route.params.id as string));

const { data: adData, loading: adLoading } = useRequest(() => getAdvertisement(adId.value), {
    refreshDeps: [adId],
});
...
const initialValues = computed(() => {
    ...
    const ad = adData.value.data;
    // We intentionally pass a dummy string to bypass z.string() initial validation issue
    // and prevent [object Object] showing in raw HTML. The real content is managed via quill Delta.
    return {
        title: ad.title,
        link: ad.link || "",
...
watch(
    ...
    (newData) => {
        if (newData) {
            durationDays.value = newData.durationDays;
            uploadedFiles.value = newData.images || [];
            if (newData.content) {
                quillDelta.value = newData.content;
                quill.value?.setContents(newData.content as Delta);
            }
        }
    }
);
...
const onSubmitEvent = async (form: FormSubmitEvent) => {
    ...
    const response = await postEditAdvertisement(
        adId.value,
        form.values.title,
        uploadedFiles.value,
...
const onEditorLoad = (event: EditorLoadEvent) => {
    ...
    if (adData.value?.data?.content) {
        quill.value?.setContents(adData.value.data.content as Delta);
    }
};
...
<template>
    <div class="flex items-center justify-center flex-col mt-24 mb-4">
        ...
        <div v-if="adLoading" class="text-center py-8">
            <ProgressSpinner />
        </div>
        <Card v-else class="xl:w-200 md:w-180 w-92">
            <template #content>
                <Form
                    v-slot="$form"
                    :resolver
                ...
                >
                    <div class="grid md:grid-cols-2 grid-cols-1 gap-4">
                        ...
                        <Button type="submit" class="md:col-span-2 col-span-1" fluid :disabled="uploadLoading || submitLoading"><PenSquare></PenSquare>Update</Button>
                    </div>
                </Form>
            </template>
        </Card>
