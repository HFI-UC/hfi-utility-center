...
const searchKeyword = ref<string | null>(
    (route.query.keyword as string) || null
);
const searchTime = ref<Date[] | null>(
    route.query.startTime && route.query.endTime
        ? [
              new Date(Number(route.query.startTime) * 1000),
              new Date(Number(route.query.endTime) * 1000),
          ]
        : null
);
...
const pageOffset = ref(route.query.page ? Number(route.query.page) * 20 : 0);
const page = computed(() => Math.floor(pageOffset.value / 20));

onMounted(() => {
    if (route.query.status) {
        const status = statusOptions.value.find(
            (s) => s.id === route.query.status
        );
        if (status) {
    ...
    zodResolver(
        z.object({
            ...
            mode: z.literal(["by-room", "single-sheet"], {
                error: t("admin.reservation.validation.modeRequired"),
            }),
        })
    )
);
...
onMounted(async () => {
    ...
    if (token && token != "") {
        ...
        if (response.success) {
            ...
            setTimeout(() => router.push("/admin/reservation"), 6500);
        }
    } else {
                            ...
                            >
                                ...
                                >
                                    <h2 class="font-bold text-xl mb-2">
                                        {{
                                            $t("admin.reservation.reservation")
                                        }}
                                        #{{ item.id }}
                                    </h2>
                                    <Fieldset>
                        ...
                        <div class="flex flex-col gap-4">
                            ...
                            <div class="grid grid-cols-9 gap-2">
                                ...
                                <Select
                                    ...
                                    v-model="searchRoom"
                                    :placeholder="
                                        $t('admin.reservation.placeholder.room')
                                    "
                                    optionLabel="name"
                                    optionValue="id"
                                    :options="rooms?.data"
