    ...
    zodResolver(
        z.object({
            name: z
                ...
                .min(1, { error: t("admin.facility.validation.nameRequired") }),
            campus: z.number({
                error: t("admin.facility.validation.campusRequired"),
            }),
        }),
    ),
);
...
const newPolicyResolver = computed(() =>
    zodResolver(
        z.object({
            ...
            startTime: z.date({
                error: t("admin.facility.validation.startTimeRequired"),
            }),
            endTime: z.date({
                error: t("admin.facility.validation.endTimeRequired"),
            }),
        }),
    ),
);
    ...
    zodResolver(
        z.object({
            ...
            name: z
                ...
                .min(1, { error: t("admin.facility.validation.nameRequired") }),
            enabled: z.boolean({
                error: t("admin.facility.validation.enabledRequired"),
            }),
        }),
    ),
);
    ...
    zodResolver(
        z.object({
            user: z.object(
                ...
                { error: t("admin.facility.validation.userRequired") },
            ),
        }),
    ),
);
...
const onNewApproverSubmit = async (form: FormSubmitEvent) => {
    ...
    loading.value = true;
    const response = await postCreateApprover(room.value, form.values.user.id);
    loading.value = false;
    if (response.success) {
        toast.add({
            severity: "success",
...
<template>
    ...
    >
        <DataTable
            :loading="roomsLoading"
            :value="
                rooms?.data.find((_room: Room) => _room.id === room)?.policies
            "
            class="text-nowrap"
        >
            <template #header>
        ...
        >
            <div class="flex flex-col gap-4">
                <Select
                    ...
                    "
                    :placeholder="$t('admin.facility.form.user')"
                    name="user"
                    fluid
                >
    ...
    >
        <DataTable
            :loading="roomsLoading"
            :value="rooms?.data.find((r: Room) => r.id === room)?.approvers"
            class="text-nowrap"
        >
            <template #header>
                <div class="flex gap-2 justify-between">
